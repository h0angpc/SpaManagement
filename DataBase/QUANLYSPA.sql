...
AS
BEGIN
	SELECT TOP (@TOP) P.PRO_NAME AS N'Mỹ phẩm', A.ToTal AS N'SL'
	FROM PRODUCT P
	JOIN 
	(
		...
		WHERE MONTH(P.DAYTIME) = MONTH(GETDATE()) AND YEAR(P.DAYTIME) = YEAR(GETDATE())
		GROUP BY PT.P_ID
	) AS A
	ON P.PRO_ID = A.P_ID
...
BEGIN
	...
	WHERE MONTH(P.DAYTIME) = @m AND YEAR(P.DAYTIME) = YEAR(GETDATE())
END
GO	
